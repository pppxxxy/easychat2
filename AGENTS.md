...
- Scheduled proactive messages ("定时主动消息") live in `plugins/proactiveMessage/android/` (Kotlin, package `com.pppxxxy.easychat2.proactive`) and are injected during prebuild by `plugins/withProactiveMessage.js` (permissions, receivers, dataSync foreground service, WorkManager dependency, `MainApplication` package registration). JS API is `src/proactiveMessage.js`; notification taps reach JS through the `ProactiveMessage:onOpenRole` event handled by `ProactiveMessageBridge` in `App.js`. Never edit the generated copies under `android/` directly.
