...
- **自带 Key，直连接口**：兼容 OpenAI、DeepSeek 或任意自建网关，应用本身不代理、不中转请求。
...
- **会话隔离**：聊天记录按角色分别存储，切换角色时中断进行中的请求并丢弃迟到回复。
...
> 本项目没有独立的 lint / typecheck / 单元测试脚本。修改后请运行 `npm run start` 并手动走通受影响的路径，再执行 `npm ci` 验证依赖完整性。

## 使用指南

应用由三个底部标签页组成：聊天 / 角色 / 设置。
...
- 每条消息下方提供「复制」「选择文本」，用户消息可「修改重发」，助手消息可「重新生成」。
- 顶部「公告」按钮随时查看免责条款；输入栏「清空」可删除当前角色的会话记录。
