...
- [x] 5. 新增 MemoryScreen 记忆页
  - [x] 5.1 渲染会话列表：角色头像、角色名、摘要、更新时间（需求 2.2、2.3）
  - [x] 5.2 按置顶优先、更新时间倒序排列（需求 2.4、2.5）
  - [x] 5.3 点击行调用 `switchSession` 并切换到聊天 Tab，同时把当前角色切为该会话所属角色（需求 2.8、3.1）
  - [x] 5.4 每行提供置顶、克隆、删除按钮并接入对应操作（需求 2.6、2.7、4.1、5.1）
  - [x] 5.5 删除与克隆二次确认，失败时 `Alert` 并保持列表（设计「错误处理」）
  - [x] 5.6 空状态展示（需求 2.2）
