...
7. 图片文件回收只删除未被会话消息或待发送附件引用的文件，启动时会清理孤儿聊天图片和表情包文件。
8. 识图附件仅接受 PNG/JPEG/WebP/GIF，图片尺寸、文件大小和 Base64 总量均在发送前校验。
...
- 相册取消选择时不改变当前表情包列表。
- 图片选择先检查文件大小，再检查尺寸；识图附件仅接受 PNG/JPEG/WebP/GIF；单文件 12 MiB、1600 万像素、单次 3 张、总文件 20 MiB、Base64 28 MiB。
