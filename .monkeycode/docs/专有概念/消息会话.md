...
数据层已按会话模型实现（`src/storage.js` 与 `src/context/sessionLibrary.js`），并保留旧键 `@easychat2_messages::<characterId>` 与 `@easychat2_messages` 的迁移读取，`migrateLegacyMessages` 将其转为 `legacy-<characterId>` 历史会话且幂等。界面层（`ChatScreen`）当前仍按角色读写消息，将在会话改造阶段切换为按 `activeSessionId` 读写。
