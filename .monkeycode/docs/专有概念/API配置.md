...
每组 API 配置由三部分组成：接口地址、模型名与密钥。应用支持多套配置，用户可以创建、切换和管理多组配置，当前选中的配置在发送请求时被使用。配置保存在设备本机，`api.js` 只读取当前活跃配置。
...
| 读写 | `src/storage.js` 的 `getApiConfigs` / `saveApiConfigs` / `getActiveApiConfig` |
| 消费方 | `src/api.js` 的 `sendChatMessage`（通过 `getActiveApiConfig`） |
...
4. **地址会被归一化**: 无论用户填写哪种形式，最终都会得到以 `/chat/completions` 结尾的地址。
5. **明文地址需用户确认**: 匹配 `/^http:\/\//i` 时，保存前必须经过确认弹窗。
