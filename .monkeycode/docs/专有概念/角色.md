...
| 角色库读写 | `src/storage.js` 的 `getCharacterLibrary` / `saveCharacterLibrary` / `saveCharacterState` |
...
| `presets` | `CharacterPreset[]` | 随角色卡保存的角色预设 | 缺失为 `[]`，启用项注入 `[角色预设]` |
...
5. **开场白按选择保存**: 单聊新建会话前选择开场白，选择结果更新角色 `firstMes` 并写入新会话；已有开场白可单独重选。
