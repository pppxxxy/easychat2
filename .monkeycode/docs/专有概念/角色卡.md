...
角色卡（Character Card）是 SillyTavern / RP-Hub 生态中通行的人物设定交换文件，包含角色名、人设、开场白、世界书与正则脚本等数据。EasyChat2 支持导入 PNG 或 JSON 格式的角色卡，快速创建带完整设定的角色。

## 什么是角色卡？

PNG 格式把 JSON 以 base64 编码写入图片的文本元数据块，JSON 格式直接暴露结构化字段。应用在 `src/cardParser.js` 中自行完成字段提取与标准化，覆盖两类生态的差异：

- **SillyTavern V2/V3**：标准字段位于 `data` 包装内，世界书位于 `data.character_book.entries`，正则位于 `data.extensions.regex_scripts`
- **RP-Hub 扁平卡**：标准字段位于顶层，世界书位于顶层 `worldInfo`，正则位于顶层 `regexScripts`
...
| 文件选取与落库 | `src/CharacterScreen.js` 的 `importCard` |
...
1. **PNG 无数据与格式错误区分处理**: PNG 未找到 `chara`/`ccv3` 文本块时提示「该图片不包含角色卡数据，请上传 RP-Hub 导出的 JSON 文件或含数据的 PNG 图片。」；文件损坏、base64 解码失败、JSON 语法错误才提示「角色卡解析失败」并附带脱敏后的错误详情。
