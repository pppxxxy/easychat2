...
- **织语结构**：`spec: "zhiyu_agent_v1"` 的 `data.nickname`、`data.greeting`、`data.setting` 分别映射到角色名、开场白、人设/系统提示词；`agent_regex`、`worldbook`/`world_book` 等字段映射到正则与世界书
- **纯文本容错**：JSON 导入兼容 BOM、代码围栏、全角空白，以及字符串内未转义换行；`user_setting`、`agent_regex_css`、模型、温度、窗口、个性化颜色等非角色字段会被忽略
...
- 导入读取、解析与保存阶段显示等待弹层；大卡片显示文件大小和“请稍候”提示
...
| `nickname` / `agent_nickname` | `name` | 织语字段优先回退到标准名称 |
| `description` | `description` | 空串 |
| `personality` | `personality` | 空串 |
| `scenario` | `scenario` | 空串 |
| `first_mes` / `firstMes` | `firstMes` | 空串 |
| `greeting` / `agent_greeting` | `firstMes` | 织语字段回退到标准开场白 |
| `setting` / `agent_setting` | `systemPrompt` | 作为人设/系统提示词导入 |
| `preset_dialogs` | `alternateGreetings` | 可识别的文本对话候选项 |
...
| `tags` / `data.tags` | `tags` | `[]` |
| `worldInfo` / `character_book` / `worldbook` / `world_book` | `worldInfo` | `[]` |
| `regexScripts` / `extensions.regex_scripts` / `agent_regex` | `regexScripts` | `[]` |
