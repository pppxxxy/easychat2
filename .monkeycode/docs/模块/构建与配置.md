...
| 文件 | 目的 |
|------|------|
| `app.json` | 声明应用名、包名 `com.pppxxxy.easychat2`、暗色主题、Android 权限与 `expo-image-picker` 图片用途文案、EAS projectId，并通过 `withAsyncStorageDbSize` 将 Android AsyncStorage 数据库上限设为 1024 MiB；`icon` / `splash` / `android.adaptiveIcon` 指向 `assets/` 下的 1024×1024 资源，自适应图标前景主体位于中央 66% 安全区、背景色 `#1a1a2e` |
...
- `expo-image-picker@~14.7.1`、`expo-image-manipulator@~11.8.0` - 系统图片选择与表情包缩放；升级后需重新验证权限、prebuild 与 Metro 打包
...
```mermaid
flowchart LR
    Checkout["Checkout 代码"] --> Node["安装 Node 22 / Java 17 / Android SDK"]
    Node --> CI["npm ci"]
