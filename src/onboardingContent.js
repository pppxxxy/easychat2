...
export const ONBOARDING_CHAPTERS = [
  ...
  },
  {
    ...
    steps: [
      ...
      '点击「导入角色卡」，选择 PNG 或 JSON 文件。',
       ...
       '导入超大角色卡时，App 会将正文保存到本地文件，避免单个 AsyncStorage 值过大；包含独立 HTML、脚本和样式的开场白会交给聊天页的富 HTML 渲染。',
    ...
    items: [
      {
         name: '导入角色卡',
         where: '角色 → 基本信息 → 导入角色卡',
         usage: '支持 V2/V3 标准的 PNG 与 JSON 角色卡，导入后自动解析人设、开场白、世界书与正则；大角色卡正文会落本地文件。',
      },
      {
        name: '角色卡来源',
      ...
      },
       {
         name: '大型 HTML 开场白',
         where: '角色卡正则脚本 / 开场白',
         usage: '可为开场白配置完整 HTML、CSS、脚本和正则替换；聊天页会使用本地文件源与可滚动 WebView 渲染。',
       },
       {
         name: '导出角色卡',
        where: '角色 → 基本信息 → 导出角色卡',
  ...
  },
  {
    ...
    steps: [
      ...
      '长按或点击消息下方的操作行，可以复制、引用或重新生成；用户文字消息的「修改重发」会先请求确认。',
    ],
    items: [
      ...
      { name: '聊天设置', where: '「⋯」菜单中的「设置」', usage: '进入聊天设置弹窗，或跳转系统设置与角色编辑。' },
       ...
       { name: '修改重发', where: '用户文字消息 → 修改重发', usage: '确认后撤回当前消息及后续回复，并把原文字回填输入框。' },
       { name: '图片与表情包', where: '输入栏附件 / 笑脸按钮', usage: '图片拆分为媒体消息，表情包可从相册添加并重复发送。' },
  ...
  },
  {
    ...
    title: '图片、表情包与大角色卡',
    icon: 'image-outline',
    summary: '发送本地图片，收藏常用表情包，并适配大型 HTML 开场白。',
    intro: '聊天页支持图片附件和表情包。普通图片会作为独立媒体消息保存，表情包可以从图片制作并重复使用；大角色卡的独立 HTML 开场白会使用更宽的渲染区域。',
    steps: [
      '在聊天输入栏左侧点击附件按钮，选择纯文本文档或图片。图片需要当前模型来源支持识图。',
      '同时输入文字并发送时，图片会先保存为媒体消息，文字随后作为第二条用户消息发送；只发送图片时不会生成空文字消息。',
      '长按聊天中的图片可以保存、保存为表情包或删除图片消息。图片文件保存在本机应用目录，删除消息后会按引用情况回收。',
      '点击输入栏右侧的透明笑脸按钮打开表情包面板，首个格子用于从相册添加表情包；选择一张图片并填写名称后保存。',
      '在「设置 → 全局配置」打开「全宽对话」时，角色头像和名字会放到消息上方，名字位于头像右侧，消息框横向铺满聊天区域。',
      '导入 PNG 或 JSON 大角色卡时，App 会把超大角色正文保存为本地文件；包含 HTML、脚本和样式的独立开场白会在聊天页使用可滚动渲染区域。',
      '点击文字消息的「修改重发」后先确认，确认会撤回该消息及其后续回复，并把原文字回填到输入框。',
    ],
    items: [
      { name: '图片附件', where: '聊天 → 输入栏左侧附件', usage: '选择图片后可在文字旁继续输入；图片单独作为媒体消息展示。' },
      { name: '图片长按操作', where: '聊天图片', usage: '保存图片、保存为表情包或删除图片消息。' },
      { name: '表情包面板', where: '聊天 → 输入栏右侧笑脸', usage: '首格添加表情包，点击已保存表情包即可发送。' },
      { name: '表情包名称', where: '表情包保存弹窗', usage: '名称会作为模型提示和历史记录中的可读标签。' },
      { name: '全宽对话', where: '设置 → 全局配置 → 全宽对话', usage: '让消息框横向占满聊天区域，角色头像和名字置于消息上方。' },
      { name: '大角色卡开场白', where: '角色 → 导入角色卡 → 聊天', usage: '支持大型 HTML、脚本、样式和正则替换开场白，使用本地文件与 WebView 渲染。' },
      { name: '修改重发', where: '文字消息 → 修改重发', usage: '确认后撤回当前文字消息及后续回复，原文字回到输入框。' },
    ],
    note: '图片和表情包只保存在本机。普通图片会检查单文件大小、分辨率、数量和总量；支持识图与否由当前 API 来源设置决定。',
  },
  {
    id: 'character-edit',
    title: '角色编辑',
...
export function getOnboardingChapter(id) {
  return CHAPTER_MAP[String(id || '')] || null;
}
